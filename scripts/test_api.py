...
def run(base):
    ...
    def t2():
        ...
        print(
            f"    assigned={r['assigned_count']}/30  vehicles={r['vehicles_used']}  dist={r['total_distance_km']} km"
        )
    ...
    def t5():
        t0 = time.perf_counter()
        r = post_json(
            f"{base}/api/v1/optimize-routes", gen_payload(100, seed=77, vehicles=10)
        )
        elapsed = time.perf_counter() - t0
        check(r["status"] == "success", f"status={r['status']}")
        print(
            f"    {elapsed:.2f}s wall | assigned={r['assigned_count']}/100 | unassigned={r['unassigned_count']}"
        )
