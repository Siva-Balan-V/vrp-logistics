...
export default function BillingPage() {
  ...
  const handlePortal = async () => {
    ...
    } catch (e) { alert('Portal failed: ' + e.message) }
  }

  const inputStyle = {
    background: 'var(--bg-2)', color: 'var(--text-1)',
    border: '1px solid var(--border)', borderRadius: 'var(--radius)',
    padding: '7px 10px', fontSize: 12, fontFamily: 'var(--mono)',
  }
        ...
        {usage && (
          <div style={{
            ...
            <h2 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12, color: 'var(--text-1)', fontFamily: 'var(--mono)' }}>
              Current Plan: <span style={{ textTransform: 'capitalize', color: 'var(--accent)' }}>{usage.plan}</span>
            </h2>
            <div style={{ display: 'flex', gap: 24, fontFamily: 'var(--mono)', fontSize: 12 }}>
              <div>
                <span style={{ color: 'var(--text-3)' }}>Optimizations this month: </span>
                <span style={{ color: 'var(--text-1)' }}>{usage.monthly_optimizations_used} / {usage.monthly_optimizations_limit}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span style={{ color: usage.export_enabled ? 'var(--green)' : 'var(--text-3)' }}>{usage.export_enabled ? 'Enabled' : 'Not available'}</span>
              </div>
            </div>
          </div>
        )}
          ...
          {plans.map((plan) => {
            ...
            return (
              <div key={plan.id} style={{
                ...
                <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-1)', fontFamily: 'var(--display)' }}>{plan.name}</h3>
                <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--accent)', fontFamily: 'var(--display)' }}>
                ...
                <div style={{ fontFamily: 'var(--mono)', fontSize: 11, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <span style={{ color: 'var(--text-2)' }}>📊 {plan.max_optimizations_per_month === 10000 ? 'Unlimited' : plan.max_optimizations_per_month} optimizations/mo</span>
                  <span style={{ color: 'var(--text-2)' }}>📍 Up to {plan.max_locations_per_job} locations/job</span>
                  <span style={{ color: 'var(--text-2)' }}>🗺 {plan.allowed_backends.join(', ')} routing</span>
                  <span style={{ color: plan.export_enabled ? 'var(--green)' : 'var(--text-3)' }}>📤 Export: {plan.export_enabled ? 'Yes' : 'No'}</span>
                  <span style={{ color: plan.priority_support ? 'var(--green)' : 'var(--text-3)' }}>🎧 Priority support: {plan.priority_support ? 'Yes' : 'No'}</span>
                  <span style={{ color: 'var(--text-2)' }}>👥 Up to {plan.max_users} users</span>
                </div>
                {plan.id !== 'free' && (
                  <button onClick={() => handleUpgrade(plan.id)} disabled={isCurrent} style={{
                    background: isCurrent ? 'var(--bg-3)' : 'var(--accent)',
                    color: isCurrent ? 'var(--text-3)' : '#000',
                    border: 'none', borderRadius: 'var(--radius)',
                    padding: '10px', fontSize: 12, fontWeight: 600,
                    cursor: isCurrent ? 'default' : 'pointer', marginTop: 'auto',
                  }}>
                    {isCurrent ? 'Current Plan' : plan.id === 'pro' ? 'Upgrade to Pro' : 'Contact Sales'}
                  </button>
                )}
              </div>
            )
